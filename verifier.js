const pokerhand = require("./pokerhand")

// pokerhand.checkRank(["hearts-8", "hearts-9", "clubs-8", "diamonds-8", "hearts-4", "hearts-7", "hearts-5"]);

// pokerhand.checkRank(["hearts-8", "hearts-9", "clubs-8", "diamonls ds-8", "hearts-4", "hearts-7", "hearts-5"]);
// pokerhand.checkRank(["hearts-8", "hearts-7", "clubs-8", "diamonds-8", "hearts-4", "hearts-6", "hearts-5"]);

// pokerhand.checkRank(["hearts-A", "hearts-3", "clubs-5", "diamonds-2", "diamonds-4", "hearts-Q", "hearts-10"]);

//ROYAL FLUSH
console.log(pokerhand.checkRank(["hearts-A", "hearts-K", "clubs-J", "diamonds-8", "hearts-J", "hearts-Q", "hearts-10"]));

//STRAIGHT FLUSH 1,2,3,4,5
pokerhand.checkRank(["hearts-6", "hearts-3", "clubs-4", "diamonds-8", "hearts-4", "hearts-5", "hearts-2"]);

// FLUSH
pokerhand.checkRank(["hearts-9", "hearts-3", "clubs-4", "diamonds-8", "hearts-4", "hearts-5", "hearts-2"]);
                      
// // FOUR OF A KIND
pokerhand.checkRank(["hearts-7", "hearts-3", "clubs-7", "diamonds-7", "spades-7", "hearts-5", "spades-4"]);

// // FULL HOUSE
pokerhand.checkRank(["hearts-4", "hearts-3", "clubs-4", "diamonds-4", "hearts-7", "clubs-5", "spades-5"]);

// // THREE OF A KIND
pokerhand.checkRank(["hearts-4", "hearts-3", "clubs-4", "diamonds-4", "hearts-7", "clubs-9", "spades-5"]);

// // TWO PAIR
pokerhand.checkRank(["hearts-4", "hearts-3", "clubs-3", "diamonds-4", "hearts-7", "clubs-9", "spades-5"]);

// // PAIR
pokerhand.checkRank(["hearts-4", "hearts-A", "clubs-4", "diamonds-6", "hearts-7", "clubs-9", "spades-5"]);

// // HIGH CARD
 pokerhand.checkRank(["hearts-4", "hearts-7", "clubs-9", "diamonds-K", "hearts-J", "clubs-2", "spades-5"]);
